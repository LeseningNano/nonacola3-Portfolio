...
## 2026-09-09 Phase 2.1 发布后复测

生产 HTML 已确认包含 `hero-poster.webp`，说明延后加载版本已经部署。使用与基线相同的脚本从当前开发机执行 7 轮只读 HTTP 测量：

| 测量项 | 优化前中位数 / P95 | 发布后中位数 / P95 | 结果 |
| --- | ---: | ---: | --- |
| 首页 | 402 / 720 ms | 246 / 725 ms | 中位数下降约 39%，P95 基本持平 |
| News 详情 | 797 / 884 ms | 824 / 919 ms | 基本持平 |
| `/api/hero` | 804 / 937 ms | 437 / 904 ms | 中位数下降约 46%，P95 接近 |
| Hero 文件 HEAD | 417 ms | 334 ms | 单次样本，仅供参考 |

所有请求均返回 200，Hero 视频仍为 8,462,314 bytes（约 8.07 MiB）。这次改动已解除页面可见性对视频 `canplay` 的依赖，并把约 20 KB 的静态 poster 放在视频之前；它没有缩小视频本身，也不会改善 News 详情的动态数据库路径。

生产浏览器自动检查在等待海外资源时超时，因此本次没有取得可信的 LCP、DOMContentLoaded 和视频开始下载时间。上述 HTTP 数值也可能受到 CDN 命中与网络波动影响；需要在目标国内网络和设备上继续采样，才能决定是否迁移媒体或数据库。
