  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-40">
      <div className="px-4 md:px-6 h-16 flex items-center justify-between">
        <Link href="/" className="font-heading font-normal text-base md:text-lg">
          {siteConfig.name}
        </Link>
          ...
          <div className="flex flex-col px-6 py-6 gap-1">
            {SECTIONS.map((s) => (
              <a
                ...
                href={`/#${s.id}`}
                onClick={(e) => handleSectionClick(e, s.id)}
                className="font-heading text-2xl py-3 text-neutral-300 hover:text-white transition-colors border-b border-neutral-900"
              >
                {s.label}
              </a>
            ))}
