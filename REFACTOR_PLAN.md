...
- Phase 2：2.1 已完成实现、验证并推送，等待 EdgeOne 部署后复测；2.2 已完成现状评估，暂不改动已有缓存实现；2.3 等待 2.1 发布后的数据再做资源区域决策。
- Phase 3 及以后：未开始。
...
状态：实现与本地验证完成并已推送，待 EdgeOne 部署后按 Phase 0 方法复测。
...
状态：待 2.1 发布后复测再决定。当前 Hero 约 8.07 MiB，数据库首次连接与复用连接差异明显，但样本不足以支持立即迁移数据库或媒体。
